...
- Added FP8 training via `torchao` (`--fp8`): body Linears (qkv / proj / w_up / w_gate / w_down) are converted to rowwise-scaled FP8 after `.to(device)` and before `torch.compile`; `lm_head` is skipped to preserve the embedding tie. BF16 autocast still wraps the forward for the non-FP8 ops.
